...
import { fetchLoggedInUserOrdersAsync, selectUsersInfoStatus, selectUsersOrders } from '../userSlice';
import { Grid } from 'react-loader-spinner';

export default function UserOrders() {
  const dispatch = useDispatch();
  const orders=useSelector(selectUsersOrders)
  const status=useSelector(selectUsersInfoStatus)
  useEffect(()=>{
    ...
    <div>
     ...
     <h3 className="text-xl my-5 font-bold tracking-tight text-red-900">Order Status {order.status}</h3>
          ...
          <div className="flex gap-x-4">
            ...
            <p className="text-sm leading-5 text-gray-500">{order.selectedAddress.pinCode}</p></div>
            </div>
            <div>
            <div>
